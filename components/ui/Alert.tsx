...
const Alert: React.FC<AlertProps> = ({ type, message, className = '', onClose }) => {
  ...
  return (
    <div className={`border rounded-md p-4 flex items-start ${typeClasses[type]} ${className}`}>
      ...
      {onClose && (
        <div className="ml-auto pl-3">
          <button
            type="button"
            className={`inline-flex rounded-md p-1 focus:outline-none focus:ring-2 focus:ring-offset-2 ${type === 'error' ? 'focus:ring-red-500' : type === 'success' ? 'focus:ring-green-500' : type === 'warning' ? 'focus:ring-yellow-500' : 'focus:ring-blue-500'}`}
            onClick={onClose}
          >
